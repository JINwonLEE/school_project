...
def load_data():
    train_path = os.path.join(INPUT_PATH, "train_data/train_data00.txt")
...
x_test_batches = x_data.reshape(-1, batch_sizes, 6)

x_data = np.asarray(train_data)
x_data = np.delete(x_data, 0, 1) #erase date
x_batches = x_data.reshape(-1,batch_sizes, 6)
index = -1
...
x = tf.placeholder(tf.float32, [num_periods, batch_sizes, 6])
y = tf.placeholder(tf.float32, [num_periods, batch_sizes, 6])
...
stacked_output = tf.layers.dense(stacked_rnn_output, 6)
#print(stacked_rnn_output.shape)
outputs = tf.reshape(stacked_output, [num_periods,batch_sizes, 6])
